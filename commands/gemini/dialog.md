---
description: Диалог с Gemini для обсуждения и достижения консенсуса
argument-hint: [тема для обсуждения с Gemini]
---

## Цель

Вести продуктивный P2P диалог с Gemini до достижения консенсуса или эскалации к пользователю.
...
```bash
.claude/scripts/gemini/new.sh "$ARGUMENTS"
```
...
После каждого ответа Gemini задай себе вопросы:
...
- При возражении — **явно признать сильные аргументы** Gemini
...
```bash
.claude/scripts/gemini/continue.sh "твой контраргумент или уточнение"
```

Скрипт автоматически:
- Добавляет `-r latest` для продолжения сессии
...
```
/gemini:dialog оцени мой план рефакторинга компонента UserProfile
```

Claude начнёт диалог с Gemini, обсудит план, при необходимости возразит или запустит синтез, и вернёт согласованный результат.
